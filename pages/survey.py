...
from backend.services.survey_service import DEFAULT_SURVEY_QUESTIONS, SURVEY_TEMPLATES, execute_survey
...
def main() -> None:
    ...
    with st.form("survey_form"):
        ...
        template_name = st.selectbox("Survey template", list(SURVEY_TEMPLATES))
        custom_questions_text = st.text_area(
            "Custom questions (optional, one per line)",
            placeholder="What would make you trust this product?\nWhat feature would you use first?",
            help="Custom questions are added to the selected template and receive the same persona-consistent simulation.",
        )
    ...
    if submitted:
        ...
        try:
            custom_questions = [
                {"id": f"custom_{index + 1}", "question": question, "type": "single_choice",
                 "options": ["Not at all", "Somewhat", "Likely", "Very likely"], "weight": 1}
                for index, question in enumerate(line.strip() for line in custom_questions_text.splitlines())
                if question
            ]
            with st.spinner("Generating survey responses..."):
                survey_results = execute_survey(
                    ...
                    survey_questions=[*SURVEY_TEMPLATES[template_name], *custom_questions],
            )
