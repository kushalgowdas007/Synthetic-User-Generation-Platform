...
from typing import Any, Dict, List, Mapping, Optional, Sequence, Callable, TypeVar, ParamSpec
...
def compute_experiment_signature(experiment: Mapping[str, Any]) -> str:
    ...
    normalized = {
        ...
        "research_objective": _normalize_str(
...
def compute_survey_signature(*args: Any, **kwargs: Any) -> str:
    ...
    if len(args) >= 5 and isinstance(args[0], str):
        ...
        questions = kwargs.get(
            "questions",
            kwargs.get("survey_questions", []),
        )
        product = str(
            kwargs.get(
                "product",
    ...
    payload = {
        ...
        "questions": [
            str(q.get("id") or q.get("question", ""))
            for q in questions
...
def compute_report_signature(*args: Any, **kwargs: Any) -> str:
    ...
    if kwargs:
        payload = {
            ...
            "has_consultant_report": bool(
                kwargs.get("has_consultant_report", False)
            ),
            "persona_count": int(
                kwargs.get("persona_count", 0) or 0
            ),
        }
        return _json_signature(payload)
    ...
    if len(args) >= 4 and isinstance(args[0], Mapping):
        ...
        payload = {
            ...
            "survey_results": survey_results or {},
            "insight_score": (
                (insights or {}).get("confidence_score", 0)
                if isinstance(insights, Mapping)
                else 0
            ),
            "product_fit_score": (
                (insights or {}).get("product_fit_score", 0)
                if isinstance(insights, Mapping)
                else 0
            ),
        }
...
def _memory_cache(
    ...
    """Framework-independent cache decorator with TTL."""
    ...
    def decorator(func: Callable[P, R]) -> Callable[P, R]:
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            ...
            if cached is not None:
                cached_time = timestamps.get(key, 0)
...
survey_analytics_cache = BoundedLRUCache(
    capacity=50,
    name="survey_analytics",
)

dashboard_aggregation_cache = BoundedLRUCache(
    capacity=50,
...
@_memory_cache(
    survey_analytics_cache,
    ttl=3600,
)
def cached_survey_analytics(
    responses_json: str,
    template_name: str,
) -> Dict[str, Any]:
...
def cached_dashboard_aggregation(
    persona_rows_json: str,
    survey_results_json: str,
) -> Dict[str, Any]:
    """Precompute dashboard analytics metrics."""
    personas = json.loads(persona_rows_json)

    survey_results = (
        json.loads(survey_results_json)
        if survey_results_json
        else None
    )
    ...
    for persona in personas:
        ...
        tech = str(
            persona.get("technology_usage", "Medium")
        ).title()

        tech_counts[tech] = (
            tech_counts.get(tech, 0) + 1
        )
    ...
    avg_quality = (
        sum(
            float(
                p.get("quality_score", 80)
            )
            for p in personas
            if isinstance(p, Mapping)
        )
        / max(1, total_personas)
    )

    response_count = (
        len(
            (survey_results or {}).get(
                "responses",
                [],
            )
        )
        if isinstance(survey_results, Mapping)
        else 0
    )
...
persona_cache = BoundedLRUCache(
    capacity=30,
    name="persona_generation",
)

survey_cache = BoundedLRUCache(
    capacity=30,
    name="survey_execution",
)

insight_cache = BoundedLRUCache(
    capacity=30,
    name="insight_clustering",
)

report_cache = BoundedLRUCache(
    capacity=20,
    name="report_generation",
)

dashboard_cache = BoundedLRUCache(
    capacity=30,
    name="dashboard_aggregation",
)